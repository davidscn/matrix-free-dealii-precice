...
namespace TestCases
{
  ...
  template <int dim>
  void
  CookMembrane<dim>::make_coarse_grid_and_bcs(Triangulation<dim> &triangulation)
  {
          ...
          if (cell->face(face)->at_boundary() == true)
            {
              if (std::abs(cell->face(face)->center()[0] - 0.0) < tol_boundary)
                cell->face(face)->set_boundary_id(clamped_mesh_id); // -X faces
