...
manualDoubleContraction(const dealii::SymmetricTensor<4, dim, Number> &A,
                        const dealii::Tensor<2, dim, Number>          &B)
{
            ...
            for (unsigned int q = 0; q < phi.n_q_points; ++q)
              {
                ...
                const SymmetricTensor<2, dim, VectorizedArrayType>
                  structural_matrix = outer_product(structural_tensor);
                const SymmetricTensor<2, dim, VectorizedArrayType> C =
                  Physics::Elasticity::Kinematics::C(F);
                // I_4 is lambda_f^2
                ...
                SymmetricTensor<2, dim, VectorizedArrayType> S =
                  dpsi_dI * structural_matrix;
                auto tau_tendon =
